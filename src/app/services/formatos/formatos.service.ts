        ...
        this.db.list('catalogos').then(catalogos => {
            if(catalogos){
                ...
                personas.forEach(o => {
                    ...
                    if(o.persona.pais && o.persona.pais.id != "" && Object.keys(o.persona.pais).length){
                        let domicilio;
                        if(o.persona.municipio && o.persona.municipio.id != "" && Object.keys(o.persona.municipio).length){
                        ...
                        if(o.persona.estado && o.persona.estado.id != "" && Object.keys(o.persona.estado).length){
...
public setDataF1007(_data){
  ...
  this.data['xNombreEmisorFirma']  = this.auth.user.nombreCompleto;
    ...
    public setDataF1516(_data) {
        ...
        if (_data.acuerdoInicio.heredar) {
            const personas = this.findHerenciaPersonas(_data, _data.acuerdoInicio)
            victimas = personas['victimas'];
        } else {
            victimas = this.findVictimas(_data);
        }
        ...
        this.data['xVictima']                 = this.getListasPersonas(victimas)['nombres'].toLocaleString();
